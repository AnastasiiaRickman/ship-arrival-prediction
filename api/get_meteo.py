...
# Функция для загрузки метео-данных
def get_meteo_data(df):
    # Преобразуем DataFrame в формат, который можно отправить
    df_dict = df.to_dict(orient='records')

    # Указываем URL сервера Flask (например, локальный или ngrok URL)
    flask_url = "http://localhost:5000/process-data"  # Или используйте ngrok URL

    try:
        # Отправляем запрос на сервер Flask
        response = requests.post(flask_url, json=df_dict)

        # Проверяем успешность ответа
        if response.status_code == 200:
            # Если запрос успешен, получаем обновленные данные
